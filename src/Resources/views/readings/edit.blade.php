    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-primary"> 
                <div class="box-body">
                    @include('churchnet::readings.partials.edit-fields')
                </div>
...
$( document ).ready(function() {
    $('.tag-select').selectize({
        ...
        $.ajax({ url: "{{url('/')}}/admin/readings/addtag/{{$reading->id}}/" + value })
        },
        onItemRemove: function(value,$item) {
        $.ajax({ url: "{{url('/')}}/admin/readings/removetag/{{$reading->id}}/" + value })
        }
    });
    $('#body').summernote({ 
        height: 250,
