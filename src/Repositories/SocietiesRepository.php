...
class SocietiesRepository extends EloquentBaseRepository
{
    ...
    {
        return $this->model->with('services', 'users', 'location')->find($id);
    }
    ...
    public function allforcircuit($circuitnumber)
    {
        ...
        return $this->model->with('circuit', 'services', 'location')->where('circuit_id', '=', $circuit->id)->where('active','yes')->get();
    }
